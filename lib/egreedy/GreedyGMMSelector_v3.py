...
class GreedyGMMSelector:
    ...
    def __init__(self, init_means,
        ...
        self.patient_clusters = patient_clusters
        #self.n_step = n_step
    ...
    def fit(self, X):
        ...
        while True:
            ...
            if nn >= self.get_n_step(len(current_means)):
                ...
                nn = 0
                if full_bic < full_bic_min:
                    full_bic_min = full_bic
                    patient_cnt = 0
                else:
                    patient_cnt += 1
            else:
            ...
            print('patient_cnt >>>', patient_cnt)
            if patient_cnt == self.patient_clusters:
                break
...
def run_greedy_gmm_trials(X, 
    ...
    for trial in tqdm(range(n_trials), desc="GreedyGMM Trials"):
        ...
        while True: # 最初のfull GMMで落ちたら、セントロイドを選びなおす
