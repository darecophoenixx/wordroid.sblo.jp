...
class SOM(Layer):
    ...
    def set_r(self, r):
        self.r = r
        self.gamma = 1 / (2 * r**2)
        qd_calced = K.cast_to_floatx(np.exp(-self.gamma * self.qd))
        #K.set_value(self.qd_calced, qd_calced)
        self.qd_calced.assign(qd_calced)
    ...
    def build(self, input_shape):
        ...
        # qd_calced
        self.qd_calced = self.add_weight(
            name='qd_calced',
            shape=self.qd.shape,
            initializer=tf.keras.initializers.Zeros(),
            trainable=False  # 学習しない
        )
...
class sksom_keras(object):
    ...
    def _make_keras_model(self, LM):
        print('sksom_keras._make_keras_model !!!')
        inp = Input(shape=(self.init_K.shape[1],), name='inp')
        l_som = SOM(map_shape=self.kshape, lm_init=LM, qd=self.qd, name='som')
        oup = l_som(inp)
    ...
    def _fit(self, i_epochs, i_r,
        ...
        som_layer = self.model.get_layer('som')
        som_layer.set_r(i_r)
...
class sksom_keras2(sksom_keras):
    ...
    def _make_keras_model(self, LM):
        print('sksom_keras2._make_keras_model !!!')

        inp = Input(shape=(self.init_K.shape[1],), name='inp')
        l_som = SOM(map_shape=self.kshape, lm_init=LM, qd=self.qd, name='som')
        oup = l_som(inp)
        model = Model(inp, oup, name='model')
        l_som.landmarks.assign(LM)
    ...
    def _calc_mean_dist(self, x):
        ...
        d2 = self.models['model_min_d'].predict(x, batch_size=self.batch_size if self.batch_size is not None else 32)
...
class som(sksom_keras2):
    ...
    def fit(self, X, y=None,
        ...
        if r is None:
            r = self.r
        self._make_keras_model(self.init_K)
        if optimizer is None:
