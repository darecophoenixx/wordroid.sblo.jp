...
class Seq(object):
    ...
    def __init__(self, dic4seq,
        ...
        self.smpl_set = set(self.smpl)
        
        self.user_list = np.array(list(self.dic4seq.keys()), dtype=str)
        self.y = [1] * self.max_num_prod
        self.neg_y = [0] * self.max_num_prod * self.num_neg
    ...
    def __getitem__(self, iuser):
        ret_user, ret_prods, ret_negs, ret_y, ret_neg_y = self.get_data(iuser)
        return ({'input_user': ret_user, 'input_prod': ret_prods, 'input_neg': ret_negs}, {'y': ret_y, 'neg_y': ret_neg_y})
    
    def get_data(self, iuser):
        user_id = self.dic4seq.to_ids_row([iuser])[0]
        prods_id, prods_id_group = self.get_prods_id_group(iuser)
        
        prod_list_neg_smpl = self.smpl_set.difference(prods_id)
        neg = self.get_neg_prod(prod_list_neg_smpl, len(prods_id_group))
        ret_y, ret_neg_y = self.get_ret_y(len(prods_id_group))
        return ([user_id]*len(prods_id_group), prods_id_group, neg, ret_y, ret_neg_y)
    
    def get_ret_y(self, nn):
        return [self.y] * nn, [self.neg_y] * nn
    
    def get_prods_id_group(self, iuser):
        ...
        prods_id_group = [[ee1 if ee1 is not None else random.choice(prods_id) for ee1 in ee] for ee in itertools.zip_longest(*[iter(prods_id)]*self.max_num_prod)] # 2021/8/12 14:27
        return prods_id, prods_id_group
    
    def get_neg_prod(self, prod_list_neg_smpl, nn):
        try:
            neg = random.sample(prod_list_neg_smpl, nn*self.num_neg*self.max_num_prod)
        except ValueError:
            neg = random.choices(list(prod_list_neg_smpl), k=nn*self.num_neg*self.max_num_prod)
        neg = np.array(neg).reshape(nn, self.max_num_prod, self.num_neg).tolist()
        return neg
...
def make_model(num_user=20, num_product=10, max_num_prod=5,
    ...
    def calc_prob(x):
        ...
        return prob
    prob = Lambda(calc_prob, name='y')([embed_prod, embed_user])
    ...
    prob_neg = Flatten(name='neg_y')(prob2)
    model = Model([input_user, input_prod, input_neg], [prob, prob_neg])
...
class WordAndDoc2vec(object):
    ...
    def make_model(self, max_num_prod=5, num_neg=1, num_features=8,
                   gamma=0.0, embeddings_val=0.1, maxnorm=None, loss_wgt_neg=0.1):
        ...
        models = make_model(num_user=self.num_user, num_product=self.num_product, max_num_prod=max_num_prod,
                   ...
                   embeddings_val=embeddings_val, maxnorm=maxnorm, loss_wgt_neg=loss_wgt_neg)
        self.models = models
