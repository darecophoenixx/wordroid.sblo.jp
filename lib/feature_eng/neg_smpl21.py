...
class Seq2(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        xtrain = {
          ...
          'input_user_neg': np.concatenate([x_train['input_user']]*2, axis=0),
          'input_user_neg_prod': np.concatenate([x_train['input_user_neg_prod'], input_user_neg_prod2], axis=0),
...
def make_model(num_user=20, num_product=10,
    ...
    input_user_neg = Input(shape=(stack_size,), name='input_user_neg')
    input_user_neg_prod = Input(shape=(stack_size, num_neg), name='input_user_neg_prod')
    
    input_prod_neg = Input(shape=(stack_size,), name='input_prod_neg')
    input_prod_neg_user = Input(shape=(stack_size, num_neg), name='input_prod_neg_user')
    ...
    embed_user_neg = user_embedding(input_user_neg)
    embed_user_neg_prod = prod_embedding(input_user_neg_prod)
    #print('embed_neg_prod >', K.int_shape(embed_neg_prod))
    
    embed_prod_neg = prod_embedding(input_prod_neg)
    embed_prod_neg_user = user_embedding(input_prod_neg_user)
    ...
    model_prod_neg_user = Model(input_prod_neg_user, embed_prod_neg_user)
    ...
    input_prod_neg_user_vec = Input(shape=(stack_size, num_neg, num_features), name='input_prod_neg_user_vec')
    ...
    #embed_neg_user_a = Lambda(lambda x: K.expand_dims(x, axis=1))(embed_neg_user)
    prob4 = Lambda(calc_prob2, name='calc_prob4', arguments={'nn1': stack_size, 'nn2': num_neg})([embed_user_neg, embed_user_neg_prod])
    #print('prob4 >', K.int_shape(prob4))
    model_prob4 = Model([input_user_neg, input_user_neg_prod], prob4, name='model_prob4')
    ...
    '''prod x prod_neg_user'''
    #embed_neg_user_a = Lambda(lambda x: K.expand_dims(x, axis=1))(embed_neg_user)
    prob5 = Lambda(calc_prob2, name='calc_prob5', arguments={'nn1': stack_size, 'nn2': num_neg})([embed_prod_neg, embed_prod_neg_user])
    #print('prob5 >', K.int_shape(prob5))
    model_prob5 = Model([input_prod_neg, input_prod_neg_user], prob5, name='model_prob5')
    
    prob5_cnfm = Lambda(calc_prob2, name='calc_prob5_cnfm', arguments={'nn1': stack_size, 'nn2': num_neg})([input_prod_vec, input_prod_neg_user_vec])
    model_prob5_cnfm = Model([input_prod_vec, input_prod_neg_user_vec], prob5_cnfm, name='model_prob5_cnfm')
    ...
    prob_prod_neg_user = Lambda(lambda x: x, name='prod_neg_user')(prob5)
    #print('prob >', K.int_shape(prob))
    model = Model([input_user, input_prod, input_user_neg, input_user_neg_prod, input_prod_neg, input_prod_neg_user], [prob1, prob_user_neg_prod, prob_prod_neg_user])
    model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['binary_accuracy'],
                  loss_weights={'y': 1.0, 'user_neg_prod': loss_wgt_neg, 'prod_neg_user': loss_wgt_neg})
    models = {
        'model': model,
        'model_user': model_user,
        'model_neg_user': model_prod_neg_user,
        'model_prod': model_prod,
        'model_neg_prod': model_user_neg_prod,
        'model_prob1': model_prob1,
        'model_prob4': model_prob4,
        'model_prob5': model_prob5,
        'model_prob1_cnfm': model_prob1_cnfm,
        'model_prob4_cnfm': model_prob4_cnfm,
        'model_prob5_cnfm': model_prob5_cnfm,
    }
    return models
