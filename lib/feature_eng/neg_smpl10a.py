...
def create_smart_csr(corpus_csc, num_docs, num_features, dtype=np.float64):
    sim = MySparseMatrixSimilarity(corpus_csc, use_getCorpusByDoc=True)
    wtsmart2_csr = gensim.matutils.corpus2csc(
        (gensim.matutils.scipy2sparse(sim.getCorpusByDoc(ii, method='WT_SMART2')) for ii in itertools.islice(np.arange(num_docs), None)),
        num_terms=num_features, dtype=dtype
        ).T
    return wtsmart2_csr
