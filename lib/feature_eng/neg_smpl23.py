...
class GaussianKernel2(Layer):
    ...
    def __init__(self, stack_size=5, num_neg=2, **kwargs):
        super(GaussianKernel2, self).__init__(**kwargs)
        self.stack_size = stack_size
        self.num_neg = num_neg
        self.output_dim = (stack_size, num_neg)
    ...
    def call(self, x, training=None):
        return self.gauss(x, K.exp(self.gamma_elm), training=training)
    
    def gauss(self, d2, gamma, training=None):
        ret = K.exp(-gamma * d2)
        return ret
...
def make_model(num_user=20, num_product=10,
    ...
    if gamma_weight is None:
      layer_g = GaussianKernel2(stack_size=stack_size, num_neg=num_neg, name='gamma', weights=[np.array([np.log(1./(num_features*sigma2))])])
    else:
      layer_g = GaussianKernel2(stack_size=stack_size, num_neg=num_neg, name='gamma', weights=[np.array([np.log(gamma_weight)])])
    ...
    d2_1 = Lambda(calc_d2, name='d2_1', arguments={'nn1': stack_size, 'nn2': 1})([embed_user, embed_prod_a])
    prob1 = layer_g(d2_1)
    #print('prob1 >', K.int_shape(prob1))
    model_prob1 = Model([input_prod, input_user], prob1, name='model_prob1')

    prob1_cnfm0 = Lambda(calc_d2, arguments={'nn1': stack_size, 'nn2': 1})([input_user_vec, Lambda(lambda x: K.expand_dims(x, axis=1))(input_prod_vec)])
    prob1_cnfm = layer_g(prob1_cnfm0)
    ...
    d2_1_col = Lambda(calc_d2, name='d2_1_col', arguments={'nn1': stack_size, 'nn2': 1})([embed_user_col, embed_prod_col_a])
    prob1_col = layer_g(d2_1_col)
    model_prob1_col = Model([input_prod_col, input_user_col], prob1_col, name='model_prob1_col')

    prob1_col_cnfm0 = Lambda(calc_d2, arguments={'nn1': stack_size, 'nn2': 1})([input_user_col_vec, Lambda(lambda x: K.expand_dims(x, axis=1))(input_prod_col_vec)])
    prob1_col_cnfm = layer_g(prob1_col_cnfm0)
    ...
    d2_4 = Lambda(calc_d2, name='d2_4', arguments={'nn1': stack_size, 'nn2': num_neg})([embed_user, embed_user_neg_prod])
    prob4 = layer_g(d2_4)
    #print('prob4 >', K.int_shape(prob4))
    model_prob4 = Model([input_user, input_user_neg_prod], prob4, name='model_prob4')

    prob4_cnfm0 = Lambda(calc_d2, name='calc_prob4_cnfm', arguments={'nn1': stack_size, 'nn2': num_neg})([input_user_vec, input_user_neg_prod_vec])
    prob4_cnfm = layer_g(prob4_cnfm0)
    ...
    d2_4_col = Lambda(calc_d2, name='d2_4_col', arguments={'nn1': stack_size, 'nn2': num_neg})([embed_user_col, embed_user_col_neg_prod])
    prob4_col = layer_g(d2_4_col)
    model_prob4_col = Model([input_user_col, input_user_col_neg_prod], prob4_col, name='model_prob4_col')

    prob4_col_cnfm0 = Lambda(calc_d2, name='calc_prob4_col_cnfm', arguments={'nn1': stack_size, 'nn2': num_neg})([input_user_col_vec, input_user_col_neg_prod_vec])
    prob4_col_cnfm = layer_g(prob4_col_cnfm0)
    ...
    y = Lambda(lambda x: x, name='y')(prob1)
    y_col = Lambda(lambda x: x, name='y_col')(prob1_col)
...
class WordAndDoc2vec(object):
    ...
    def make_model(self, num_neg=2, num_features=8,
                   ...
                   loss_wgt_y_col=0.1, loss_wgt_user_neg_prod=1.0, loss_wgt_user_col_neg_prod=0.1,
                   wgt_user=None, wgt_prod=None, gamma_weight=None):
        self.num_neg = num_neg
        self.stack_size = stack_size
        ...
        models = make_model(num_user=self.num_user, num_product=self.num_product,
                            ...
                            wgt_user=wgt_user, wgt_prod=wgt_prod, gamma_weight=gamma_weight)
        self.models = models
